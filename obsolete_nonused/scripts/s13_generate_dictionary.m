...
dictionary_full_traj = cell(traj_n_nodes, 1);
for k = 1:traj_n_nodes
    dictionary_full_traj{k} = zeros(max_muap_len, n_channels, N);
    for m = 1:N
        temp_muap = [MUs(m).muap; zeros(max_muap_len-size(MUs(m).muap,1),size(MUs(m).muap,2))];
        temp_traj = traj_mixing_mat((k-1)/(traj_n_nodes-1+eps), traj_n_nodes, n_channels);
        dictionary_full_traj{k}(:,:,m) = temp_muap * temp_traj' * electrode_diff_mat';
    end
end
...
% Full dictionary of forms in trajectory nodes
dictionary_detectable_traj = cell(traj_n_nodes, 1);
for k = 1:traj_n_nodes
    dictionary_detectable_traj{k} = zeros(max_muap_len, n_channels, numel(detectable_ind));
    for m = 1:numel(detectable_ind)
        ...
        temp_traj = traj_mixing_mat((k-1)/(traj_n_nodes-1+eps), traj_n_nodes, n_channels);
        dictionary_detectable_traj{k}(:,:,m) = temp_muap * temp_traj' * electrode_diff_mat';
    end
end
...
% dictionary_full_init = zeros(max_muap_len, n_channels, N);
% for m = 1:N
%     temp_muap = [MUs(m).muap; zeros(max_muap_len-size(MUs(m).muap,1),size(MUs(m).muap,2))];
%     temp_traj =  traj_mixing_mat(0, traj_n_nodes, n_channels);
%     dictionary_full_init(:,:,m) = temp_muap * temp_traj' * electrode_diff_mat';
% end

% dictionary_detectable_init = zeros(max_muap_len, n_channels, numel(detectable_ind));
% for m = 1:numel(detectable_ind)
%     temp_ind = detectable_ind(m);
%     temp_muap = [MUs(temp_ind).muap; zeros(max_muap_len-size(MUs(temp_ind).muap,1),size(MUs(temp_ind).muap,2))];
%     temp_traj = traj_mixing_mat(0, traj_n_nodes, n_channels);
%     dictionary_detectable_init(:,:,m) = temp_muap * electrode_diff_mat';
% end
