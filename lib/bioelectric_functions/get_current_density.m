function iap = get_current_density(t, z, zi, L1, L2, v, d, suppress_endplate_density, endplate_width)
...
psi1 = -4*get_tm_current_dz(-2*(Z-zi-v*T));
psi2 =  4*get_tm_current_dz(-2*(-Z+zi-v*T));
...
% My version:
right_wave = diff(psi1) / dz;
right_wave = right_wave .* tendon_terminator(Z(1:end-1,:)-zi-L1/2, L1);
right_wave = right_wave .* ((Z(1:end-1, :) - zi)/v > 0); % Negative time suppression

%left_wave = diff(psi2) / dz;
left_wave = -diff(psi2(end:-1:1, :))/dz;
left_wave = left_wave(end:-1:1, :);
left_wave = left_wave .* tendon_terminator(Z(1:end-1,:)-zi+L2/2, L2);
